...
class Analyser:
    ...
    def __init__(self, model, template_set='tweak_fsps_QSF_12_v3'):
        ...
        self.cat = h5py.File(f'data/{model}.h5', 'r')
        self.z = self.cat['input/z'][()]
        self.pz = self.cat[f'pz/eazy/{template_set}']
        self.za = self.pz['z_a'][:]
        self.dz = (self.z - self.za)/(1+self.z)
    ...
    def printi(self, i):
        ...
        print(self.z[i], self.za[i], '|', [
              np.round(self.pz[f'INT_ZGT{z}'][i], 2) for z in np.arange(0, 15, 1)])
    ...
    def dz_plot(self, tag=''):
        ...
        hist, bin_edges_x, bin_edges_y = np.histogram2d(
            self.z, self.dz, range=[range_x, range_y], bins=(100, 100))

        ax.imshow(hist.T, origin='lower', aspect='auto', extent=[
                  *range_x, *range_y], cmap=cmr.sunburst_r)
